const questionBank = [
    {
        id: "q1",
        type: "mc",
        questionText: `
            <b>Text 1: A MORE AGED POPULATION THAN EVER BEFORE</b>
            <br><br>
            We won’t just be wrestling with the fact that the world’s population is exploding – but people are living longer than ever, too. Which is great – but all those senior citizens are going to require care. In fact, the number of centenarians will increase more than 50 times – from 500,000 today to over 26 million by 2100. From the UK to Japan to China, societies with large numbers of people over 65 will become more common. In the next couple of decades, as that increase starts to happen, we’ll need better care for the elderly (Japan is even eyeing robots) and perhaps policies to allow more immigrants to try and make up for ageing workforces and in some cases, declining birth rates.
            <br><br>
            <b>Question:</b> According to the text, what is one potential policy solution mentioned to address the issue of aging workforces?
        `,
        options: [
            { letter: "A", text: "(A) Building more robotic caretakers for the elderly." },
            { letter: "B", text: "(B) Encouraging higher birth rates in countries like Japan." },
            { letter: "C", text: "(C) Allowing more immigration." },
            { letter: "D", text: "(D) Increasing the number of centenarians to 26 million." },
            { letter: "E", text: "(E) Expanding the population in the UK and China." }
        ],
        correctAnswer: "C",
        videoUrl: "",
        hints: [ 
            "Hint 1: The text mentions a policy idea to balance aging workforces.",
            "Hint 2: Think about who could fill the gap caused by an older population.",
            "Hint 3: It involves people moving from other countries."
        ]
    },

    {
        id: "q2",
        type: "mc",
        questionText: `
            <b>Text 2: THE RISE OF URBAN LIVING</b>
            <br><br>
            By 2050, nearly 70% of the world’s population will live in cities. This rapid urbanization brings both opportunities and challenges. While cities can offer better jobs, healthcare, and education, they also face problems such as pollution, housing shortages, and increased traffic. Many governments are now investing in smart city technologies—using data and digital tools to make urban areas more efficient and sustainable.
            <br><br>
            <b>Question:</b> According to the text, how are many governments responding to the challenges of rapid urbanization?
        `,
        options: [
            { letter: "A", text: "(A) By encouraging people to move to rural areas." },
            { letter: "B", text: "(B) By investing in smart city technologies." },
            { letter: "C", text: "(C) By reducing healthcare and education in cities." },
            { letter: "D", text: "(D) By banning private vehicles in all major cities." },
            { letter: "E", text: "(E) By building more factories and industries in urban areas." }
        ],
        correctAnswer: "B",
        videoUrl: "",
        hints: [ 
            "Hint 1: The text mentions the use of data and digital tools.",
            "Hint 2: The goal is to make cities more efficient and sustainable.",
            "Hint 3: Think of technology-based solutions to urban problems."
        ]
    }
];
